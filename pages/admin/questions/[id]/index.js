...
export default function Question () {
    ...
    query GetQuestionByID {
      questionById(id: "60b91011374b743b24408da1")
      {
        text
        questionLi
        prevQuestionLi
...
const UPDATE = gql`
mutation updateQuestion($id: ID!, $name: String!, $description: String!, $imgURL: String!, $category: String!, $color: Color!, $price: String!) {
  updateSock(sockId: $id,  sock:{name: $name, description: $description, imgURL: $imgURL, category: $category, color: $color, price: $price, total: 1}){
  id name description imgURL addedOn editedOn category color price total
}}
`;
...
////////////////////////////////////////////////////////////
  ...
  const { loading, error, data } = useQuery(GET_QUESTIONBYID);
  ...
  return (
    <>
        ...
        <>
            <div className="mx-auto my-2 w-fit text-center">
                <h1 className="text-2xl">{data.questionById.questionLi}</h1>
...
{/* 
                    ...
                    <label className="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold mb-1">Upload Photo</label>
                        <div className='flex items-center justify-center w-full'>
                            ...
                            <input onChange= {(e)=> {setImage(e.target.files[0]); console.log(e.target.files[0])}} type='file' className="hidden" />
                            </label>
