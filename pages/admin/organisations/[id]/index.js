...
export default function Organisation({}) {
    ...
    useEffect(() => {
      if (typeof window !== 'undefined') {
        ...
        setOrgId(m.replace('/admin/organisations/',''))
    ...
    const GET_ORGANISATIONBYID = gql`
    query organisationById {
      organisationById(id: ${JSON.stringify(orgId)})
      {
        text
        title
        description
  ...
  `;
    const org = useQuery(GET_ORGANISATIONBYID);
    
    const UPDATE = gql`
    mutation updateOrganisation($id: ID!, $text: String!, $title: String!,  $organisationLi: String!, $description: String!, $imgURL: String!, $longitude: String!, $latitude: String!) {
    updateOrganisation(organisationId: $id, organisation:{text: $text, title: $title, imgURL: $imgURL , organisationLi: $organisationLi, description: $description, longitude: $longitude, latitude: $latitude}){
     title 
    }}
    `;
    ...
    {user && <>
      <Formik
        ...
        onSubmit={(values, { setSubmitting, resetForm }) =>{
            setTimeout(() => {
              updateItem({ variables: { id: orgId, title: values.title, description: values.description, text: values.text, 
      ...
      >
          {props => (
              ...
              <div className="flex justify-center py-4">
                    ...
                    <div className="flex">
                        <h1 className="text-gray-600 font-bold md:text-2xl text-xl">Edit an organisation</h1>
                    </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 mt-5 mx-7">
                    <CustomTextInput label="Organisation number" name="organisationLi" placeholder="organisation001" />
                    </div>
                    <div className="grid grid-cols-1 mt-5 mx-7">
